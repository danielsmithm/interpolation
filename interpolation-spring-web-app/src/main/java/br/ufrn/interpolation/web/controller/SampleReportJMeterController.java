...
public class SampleReportJMeterController {
    ...
    private SampleReporter createSampleReporter(CsvParser parsingStrategy) throws IOException {
        SensorParser sensorParser = new SensorParserImpl(parsingStrategy);
        SampleParser sampleParser = new SampleParserImpl(parsingStrategy);

        return new SampleReporter(createSampleRepository(sampleParser), createSensorRepository(sensorParser));
    }

    private SampleRepository createSampleRepository(SampleParser sampleParser) throws IOException {
        return new SampleInMemoryRepository(sampleParser.parseFile(Paths.get("src","main","resources", "samples", "data.csv")));
    }

    private SensorRepository createSensorRepository(SensorParser sensorParser) throws IOException {
        return new SensorInMemoryRepository(sensorParser.parseFile(Paths.get("src","main","resources", "samples", "sensors.csv")));
    }
