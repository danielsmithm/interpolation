...
public class CassandraSparkSampleRepository implements SampleRepository {
    ...
    private void executeBatch(Collection<Sample> sampleCollection, Session session, String s) {
        ...
        session.execute(preparedStatement.bind(sampleCollection.stream()
                .map(sample -> sensorInsertToObjectArray(sample))
                .flatMap(Stream::of)
                .toArray()));
    }

    private Object[] sensorInsertToObjectArray(Sample sample) {
        return new Object[]{sample.getSensorName(), sample.getVariable(), sample.getUnits(), java.sql.Timestamp.valueOf(sample.getTime()), sample.getValue()};
    }
